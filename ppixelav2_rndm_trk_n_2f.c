...
/* Use the Delta x and Delta y input parameters to specify the hit region at the midplane */
...
//    int detect(float, float, float, int, union vect_or_f [NCRRC][2][NEHSTORE], int, float *, float [TXSIZE][TYSIZE]);
...
/* Delta x/y = 1, 3, 5 covers 1, 3 5 pixel regions about the array center */
			   
		   vect[0] = deltaxlen * xsize * (rvec[0] - 0.5) + (vect[2] - thick/2.)*locdir[0]/locdir[2];
		   vect[1] = deltaylen * ysize * (rvec[1] - 0.5) + (vect[2] - thick/2.)*locdir[1]/locdir[2];
...
/*  Determine current time */
          ...
          yday = (*nows).tm_yday;
		   ...
		   if(ievent < 25) {printf("day %d at %02d:%02d:%02d, run %d, event %d, number of e-h pairs = %d\n", yday, hour, min, sec, irun, ievent, neh);
		      printf("midplane x/y = %f/%f\n", vect[0]+thick*vect[3]/vect[5]/2., vect[1]+thick*vect[4]/vect[5]/2.);
		   }
...
/* Subroutine */ int deposit(vect, thick, new_drde, nmax, xeh, neh)
float *vect, thick;
int new_drde, nmax;
vect_or_f xeh[2][NEHSTORE];
int *neh;
{
    /* Initialized data */
    ...
    /* Builtin functions */
    double acos(), sqrt(), atan2(), log(), sin(), cos();
	
    /* Local variables */
    extern double drde();
	extern double rutherford();
    ...
    static int ndelta;
    static float pathmf;
    extern /* Subroutine */ int ranlux_(), newdir(), rnpssn_();
    static float phi;
    static int ipr;
    ...
    if (fcall) {
		...
		/* Assume 45 GeV pion energy if direction vector is normalized to 1 */
		
		if(ppion < 1.1) {ppion = 45.;}
...
/* Subroutine */ int propag(thick, xsize, ysize, temp, flux, rhe, rhh, max_drift_time, stimstp, ehole, neh, xeh, indeh, xhisteh)
float thick, xsize, ysize, temp, flux[2], rhe, rhh, max_drift_time, stimstp;
int neh, ehole;
vect_or_f xeh[2][NEHSTORE];
vect_or_f xhisteh[NCRRC][2][NEHSTORE];
int indeh[2][NEHSTORE];
{
	/* Initialized data */
	...
	static int nindeh[2];
	extern /* Subroutine */ int fieldsav();
	static float bconst, cconst, dconst, econst;
	static int nshift;
	extern /* Subroutine */ int triplg();
	static int ntpart;
...
double eloss(ppion, prob, path)
float ppion, prob, *path;
{
    /* Initialized data */
...
/*  The flag for first call */
    ...
    if (fabs((double)(ppion-ppold)) > 0.01) {
...
float ppion, einlog;
{
    ...
    /* Local variables */
	static double elog;
    static float gamma2, gb, beta2, emax, xruthr, scale;
...
/* Subroutine */ int newdir(v, cost, phi)
float cost, phi;
vect_or_f *v;
{
    /* Initialized data */
...
/* Subroutine */ int crrc(peaktim, samptim, stimstp, respfcn)
float peaktim, samptim, *stimstp, respfcn[NCRRC];
{
	/* Initialized data */
...
/* Subroutine */ int detect(xsize, ysize, thick, ehole, xhisteh, neh, respfcn, pixel, pixhist)
float xsize, ysize, thick, respfcn[NCRRC], pixel[TXSIZE][TYSIZE], pixhist[NCRRC+1][TXSIZE][TYSIZE];
vect_or_f xhisteh[NCRRC][2][NEHSTORE];
int neh, ehole;
{
    /* System generated locals */
...
/* Subroutine */ int fieldsav(xp, etot, ef, bf, thick, xsize, ysize)
vect_or_f *xp, *ef, *bf;
float *etot;
float thick, xsize, ysize;
{
    /* Initialized data */
...
/* Subroutine */ int wgtpotsav(xp, wgtmat, thick, xsize, ysize)
vect_or_f *xp;
float thick, xsize, ysize, wgtmat[3][3];
{
    /* Initialized data */
...
/* Subroutine */ int pixinit(pimom, thick, xsize, ysize, temp, flux, rhe, rhh, peaktim, samptim, ehole, new_drde, filebase)
float *xsize, *ysize, *thick, *temp, flux[2], *rhe, *rhh, *peaktim, *samptim, *pimom;
int *ehole, *new_drde, *filebase;
{
...
/* Subroutine */ int runinit(initseed, fileoff, runsize, lenxmin, lenxmax, deltaxlen, lenymin, lenymax, deltaylen)
float *lenxmin, *lenxmax, *deltaxlen, *lenymin, *lenymax, *deltaylen;
int *initseed, *fileoff, *runsize;
{
    /* Initialize everything from external file */
